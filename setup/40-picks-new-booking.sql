-- Parking Ops — database part 40: re-imported PICKS mark new cars NEW BOOKING.
--
-- When a PICKS sheet is imported again (the booking system sends an updated
-- file), a car that wasn't in it before is marked NEW BOOKING, the same mark
-- the office can set by hand in the car's panel, with the time it came in.
-- The first import of a day marks nothing. The IMPORT log says how many.
-- delete_sheet: a NEW BOOKING mark alone doesn't count as work on a car.
-- Otherwise the same as part 36 (import) and part 33 (delete). Safe to run twice.
...
begin
  ...
  select count(*) filter (where sent_at is not null or called_at is not null or cleared_at is not null
                            or intake <> '' or pt_at is not null or pick_called not in ('', 'New Booking') or yard <> ''),
